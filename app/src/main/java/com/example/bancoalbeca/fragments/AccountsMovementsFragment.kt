...
class AccountsMovementsFragment : Fragment(), MovementsListener {
    ...
    fun mostrarDetalle(cuenta: Cuenta) {
        val mbo: MiBancoOperacional? = MiBancoOperacional.getInstance(requireContext())

        val movimientosRaw = mbo?.getMovimientos(cuenta)
        val movimientos = ArrayList(movimientosRaw?.filterIsInstance<Movimiento>() ?: emptyList())
